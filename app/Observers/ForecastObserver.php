...
class ForecastObserver{
    ...
    public function forecast($actualData){
        $actuals = ActualData::where('product_id', $actualData->product_id)->orderBy('year', 'ASC')->orderBy('month_id', 'ASC')->get()->toArray();

        if($actuals){
            $this->forecast->where('product_id', $actuals[0]['product_id'])->delete();
            ...
            $forecast = Forecast::where('product_id', $actualData->product_id)->get();
            // Log::info($forecast);
